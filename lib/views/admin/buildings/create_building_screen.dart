...
class _CreateBuildingScreenState extends State<CreateBuildingScreen> {
  ...
  _getPosition() async {
    ...
    BuildingsCubit.get(context).lat = position.latitude;
    BuildingsCubit.get(context).long = position.longitude;
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text("Create Buildings")),
      body: Padding(
          ...
          child: Form(
            ...
            child: Column(
              children: [
                ...
                SizedBox(height: 50),
                BlocConsumer<BuildingsCubit, BuildingsState>(
                  listener: (context, state) {
                    if (state is ErrorBuildingsState) {
                      context.showErrorSnack("Error, Please try again");
                    } else if (state is SuccessBuildingsState) {
                          ...
                          child: ElevatedButton(
                            ...
                            onPressed: () {
                              if (globalKey.currentState?.validate() ?? false) {
                                BuildingsCubit.get(
                                  context,
                                ).createBuilding(nameController.text);
                              }
                            },
                            child: Text(
                              "Create Building",
